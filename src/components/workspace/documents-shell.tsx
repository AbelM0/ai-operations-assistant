...
function fileType(document: WorkspaceDocument, t: (key: string) => string) {
  if (document.mimeType === "application/pdf") return t("documents.typePdf");
  if (document.mimeType.startsWith("image/")) return t("documents.typeImage");
  return t("documents.typeDocument");
}
          ...
          for (const document of payload.documents) {
            ...
            if (previousStatus && previousStatus !== document.status) {
              if (document.status === "READY") {
                toast.success(t("documents.readyToast"), {
                  description: t("documents.readyToastDescription", {
                    name: document.originalName,
                  }),
                });
              } else if (document.status === "FAILED") {
                toast.error(t("documents.failedToast"), {
                  description:
                    document.errorMessage ||
                    t("documents.failedToastDescription", {
                      name: document.originalName,
                    }),
                });
              }
      ...
      <div className="relative lg:pl-64">
        <header className="sticky top-0 z-20 flex h-17 items-center justify-between border-b border-white/8 bg-[#050505]/88 px-4 backdrop-blur-xl sm:px-7 lg:px-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-white">{t("nav.documents")}</p>
              <p className="hidden text-xs text-[#71717A] sm:block">{t("documents.headerDescription")}</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <LanguageToggle />
            ...
            <>
              <section className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
                <div className="max-w-3xl">
                  <p className="font-mono text-[10px] font-semibold uppercase tracking-[0.2em] text-[#5EEAD4]">{t("documents.library")}</p>
                  <h1 className="mt-4 text-balance text-[clamp(2.4rem,5vw,4.6rem)] font-medium leading-[0.98] tracking-[-0.05em] text-white">{t("documents.heroTitle")}</h1>
                  <p className="mt-5 max-w-xl text-base leading-7 text-[#A1A1AA] sm:text-lg">{t("documents.heroDescription")}</p>
                </div>
              ...
              <section aria-labelledby="document-table-heading" className="mt-10 overflow-hidden rounded-xl border border-white/10 bg-[#0B0B0D]">
                <div className="flex flex-col gap-4 border-b border-white/8 px-5 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-7">
                  <div>
                    <h2 id="document-table-heading" className="text-lg font-semibold tracking-[-0.025em] text-white">{t("documents.uploadedDocuments")}</h2>
                  ...
                  <label className="flex h-10 w-full items-center gap-2 rounded-lg border border-white/10 bg-[#08080A] px-3 text-[#71717A] focus-within:border-[#2DD4BF]/45 sm:w-72">
                    ...
                    <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t("documents.searchPlaceholder")} className="min-w-0 flex-1 bg-transparent text-sm text-white outline-none placeholder:text-[#52525B]" />
                  </label>
                </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full min-w-[780px] border-collapse text-left">
                    <thead className="bg-[#0E0E11] font-mono text-[9px] uppercase tracking-[0.12em] text-[#71717A]">
                      <tr>
                        <th className="px-7 py-3 font-medium">{t("documents.columnDocument")}</th>
                        <th className="px-4 py-3 font-medium">{t("documents.columnType")}</th>
                        <th className="px-4 py-3 font-medium">{t("documents.columnSize")}</th>
                        <th className="px-4 py-3 font-medium">{t("documents.columnUploaded")}</th>
                        <th className="px-4 py-3 font-medium">{t("documents.columnStatus")}</th>
                        <th className="px-7 py-3 text-right font-medium">{t("documents.columnActions")}</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-white/7">
                      {filteredDocuments.map((document) => (
                        <tr key={document.id} className="transition-colors hover:bg-white/[0.025]">
                          ...
                          <td className="px-4 py-4 text-xs text-[#A1A1AA]">{fileType(document, t)}</td>
                          <td className="px-4 py-4 text-xs text-[#A1A1AA]">{formatBytes(document.sizeBytes)}</td>
                          <td className="px-4 py-4 text-xs text-[#A1A1AA]">{formatDate(document.createdAt, locale)}</td>
                  ...
                  {filteredDocuments.length === 0 ? (
                    <div className="flex min-h-40 flex-col items-center justify-center px-6 text-center">
                      ...
                      <p className="mt-3 text-sm font-medium text-[#D4D4D8]">{t("documents.noSearchResults")}</p>
                    </div>
                  ) : null}
                </div>
              </section>
      ...
      <Dialog open={uploadDialogOpen} onOpenChange={setUploadDialogOpen}>
        <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto border border-white/10 bg-[#0B0B0D] p-5 text-white shadow-[0_32px_120px_rgba(0,0,0,0.75)] sm:max-w-2xl sm:p-7">
          <DialogHeader className="pr-10">
            ...
            <DialogDescription className="leading-6 text-[#8B8B95]">{t("workspace.uploadDescription")}</DialogDescription>
          </DialogHeader>
  ...
  return (
    <>
      <section className="max-w-3xl">
        <p className="font-mono text-[10px] font-semibold uppercase tracking-[0.2em] text-[#5EEAD4]">{t("documents.startLibrary")}</p>
        <h1 className="mt-4 text-balance text-[clamp(2.4rem,5vw,4.6rem)] font-medium leading-[0.98] tracking-[-0.05em] text-white">{t("documents.emptyTitle")}</h1>
        <p className="mt-5 max-w-xl text-base leading-7 text-[#A1A1AA] sm:text-lg">{t("documents.emptyDescription")}</p>
      </section>
...
function DocumentStatus({
  ...
  return (
    <span title={isFailed ? errorMessage || t("documents.processingFailed") : undefined} className={`inline-flex rounded-md border px-2.5 py-1 font-mono text-[9px] uppercase tracking-[0.1em] ${isReady ? "border-[#2DD4BF]/25 bg-[#2DD4BF]/8 text-[#5EEAD4]" : isFailed ? "border-red-400/20 bg-red-400/8 text-red-300" : "border-white/10 bg-white/5 text-[#A1A1AA]"}`}>
  ...
  return (
    <>
      ...
      <nav aria-label={t("nav.workspaceNavigation")} className="mt-10 space-y-1">
        <Link href="/workspace" className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm text-[#8B8B95] transition-colors hover:bg-white/5 hover:text-white"><House className="h-4 w-4" />{t("nav.overview")}</Link>
        <Link href="/workspace/documents" aria-current="page" className="flex items-center gap-3 rounded-lg bg-[#2DD4BF]/10 px-3 py-2.5 text-sm text-[#5EEAD4]"><FileText className="h-4 w-4" weight="fill" />{t("nav.documents")}</Link>
        <Link href="/workspace/ask" className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm text-[#8B8B95] transition-colors hover:bg-white/5 hover:text-white"><ChatCenteredDots className="h-4 w-4" />{t("nav.askNexus")}</Link>
      </nav>
      <div className="mt-auto rounded-xl border border-white/8 bg-[#0D0D0F] p-4">
        <p className="font-mono text-[9px] uppercase tracking-[0.14em] text-[#5EEAD4]">{t("documents.stored", { count: documentCount })}</p>
        <p className="mt-2 text-xs leading-5 text-[#71717A]">{t("documents.storedHint")}</p>
        <button type="button" onClick={onUpload} className="mt-4 inline-flex items-center gap-2 text-xs font-semibold text-[#D4D4D8] hover:text-white"><Plus className="h-3.5 w-3.5" />{t("documents.uploadDocument")}</button>
      </div>
    </>
  );
}
