...
export function FirstSourceGuide({
  ...
  const canOpen = isReady || isFailed;
    ...
    >
      <div>
        <div className="flex items-center gap-3">
          <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-[#2DD4BF]/10 text-[#5EEAD4]">
            {isReady ? (
              ...
              <WarningCircle className="h-4.5 w-4.5 text-red-300" weight="duotone" />
            ) : (
          ...
          <div>
            <p className="font-mono text-[9px] uppercase tracking-[0.15em] text-[#5EEAD4]">
              {t(
                isReady
                  ...
                  : isFailed
                    ? "workspace.firstSource.stepFailed"
            ...
            >
              {isReady
                ...
                : isFailed
                  ? t("workspace.firstSource.failedTitle")
        ...
        <p className="mt-4 max-w-2xl text-sm leading-6 text-[#7E8C89]">
          {isReady
            ...
            : isFailed
              ? document.errorMessage || t("workspace.firstSource.failedBody")
      ...
      {canOpen ? (
        ...
        <button
          ...
          className="inline-flex h-10 w-fit cursor-wait items-center gap-2 rounded-lg border border-white/8 px-4 text-xs font-semibold text-[#52525B]"
        >
          {t("workspace.firstSource.processing")}
          <Lightning className="h-3.5 w-3.5 animate-pulse" />
        </button>
      )}
    </section>
  );
