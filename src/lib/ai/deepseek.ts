...
export const createDeepSeekCompletion = traceable(
  async function createDeepSeekCompletion({
    ...
    if (!apiKey) {
      throw new Error(
        "DeepSeek is not configured. Add DEEPSEEK_API_KEY to the server environment.",
      );
    }
  ...
  },
  {
    ...
    tags: [
      ...
      "operation:deepseek-chat-completion",
      "provider:deepseek",
    ],
    metadata: {
      application: "ai-operations-assistant",
